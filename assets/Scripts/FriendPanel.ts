...
export class FriendPanel extends Component {
  ...
  public async refreshFriendCandidates(): Promise<void> {
    const friends = await DataManager.getFriends().catch(() => []);
    console.log("[FriendPanel] 好友數 =", friends.length);

    const out: PlayerData[] = [];
    for (const f of friends) {
      const pub = await DataManager.getPublicPlayerData(f.userId).catch(() => null);
      if (!pub) continue;
      console.log("[FriendPanel] 公開資料", f.userId, "fishList # =", pub.fishList?.length ?? 0);
      // 注意：這裡需要的是「含 fishList 的 PlayerData」
      out.push(pub as any as PlayerData);
    }
    this._friends = out;
    console.log("[FriendPanel] 已快取含 fishList 的好友 =", this._friends.length);
    this.node.emit("friends-refreshed", this._friends);
  }
  ...
  public getFriendMarriageCandidates(): { ownerGameId: string; fish: FishData }[] {
    ...
    console.log("[FriendPanel] getFriendMarriageCandidates 回傳 =", out.length, out);
    return out;
  }
  ...
  /** 分頁切換 */
  private onClickFriendsTab() {
    this.switchTab(TabKind.Friends);
  }
  private onClickExploreTab() {
    this.switchTab(TabKind.Explore);
  }
        ...
        async () => {
          try {
            ...
            if (
              ...
              code === "friend_limit_reached_sender" ||
              code === "friend_limit_reached_recipient"
            ) {
              this.setInviteBtn(inviteBtn, "好友已滿", false);
