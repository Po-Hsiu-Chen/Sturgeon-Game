...
export class DecorationEditor extends Component {
  ...
  private isBgSku(sku: string) {
    return sku.startsWith("bg_") || sku === "bg_default";
  }
  private isDecoSku(sku: string) {
    return sku.startsWith("deco_");
  }
    ...
    return new Promise((resolve) => {
      ...
      resources.load(`icons/${key}/spriteFrame`, SpriteFrame, (err, sf) => {
        ...
        resources.load(`icons/${key}`, SpriteFrame, (err2, sf2) => {
          ...
          // 對背景先用 loadDir 抓第一張 frame
          resources.loadDir(`backgrounds/${key}`, SpriteFrame, (eList, list) => {
            if (!eList && list && list.length > 0) {
              list.sort((a, b) => a.name.localeCompare(b.name));
              return resolve(list[0]); // 用第一張當縮圖
            }
          });
        });
      });
  ...
  private async applyBackground(bgId: string) {
    ...
    if (bgNode) {
      ...
      if (animator) {
        // bg_default → 傳 undefined 代表用預設
        animator.setBackground(bgId === "bg_default" ? undefined : bgId);
      } else {
        // 沒掛動畫時，就用原本的靜態邏輯
        const sp = bgNode.getComponent(Sprite);
        if (sp) {
          if (bgId === "bg_default") {
            sp.spriteFrame = (this.gm as any)["defaultBackgroundSpriteFrame"];
          } else {
