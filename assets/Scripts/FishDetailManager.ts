...
export class FishDetailManager extends Component {
  ...
  switchTab(tabName: "feed" | "heal" | "fashion") {
    ...
    if (tabName === "fashion" && !this.isReadOnly) {
      this.getCurrentFishAndPlayer().then(({ fish }) => {
        if (fish) this.renderFashionGrid(fish);
      });
    }
  }
  ...
  private async onClickMarry() {
    console.log("[MarryClick][v2] onClickMarry 被按下, currentFishId =", this.currentFishId);
    ...
    // ===== 0) 先把基本環境印出來：currentUserId / apiBase =====
    // 若 currentUserId 是空，getFriends() 一定回 []
    console.log(
      "[MarryClick][v2] currentUserId =",
      (DataManager as any).currentUserId,
      "apiBase =",
      (DataManager as any).apiBase
    );

    // ===== 1) 抓好友清單 =====
    let friendCandidatesRaw: Array<{ ownerGameId: string; fish: FishData }> = [];
    try {
      const friends = await DataManager.getFriends().catch((e) => {
        console.warn("[MarryClick][v2] getFriends 失敗：", e);
        return [];
      });
      console.log("[MarryClick][v2] 好友數 =", friends.length, friends); // ← 必須看到

      // 若這裡是 0，請先確認真的互加成好友，或檢查 currentUserId 是否有設到（DataManager.init）
      // getFriends 的實作會打 /friends/:userId 需要 currentUserId。:contentReference[oaicite:2]{index=2} :contentReference[oaicite:3]{index=3}

      // ===== 2) 對每位好友抓公開玩家資料（會帶 fishList）=====
      for (const fr of friends) {
        const pub = await DataManager.getPublicPlayerData(fr.userId).catch((err) => {
          console.warn("[MarryClick][v2] 取好友公開資料失敗", fr.userId, err);
          return null;
        });
        if (!pub) continue;

        const ownerGameId = (pub as any).gameId ?? fr.userId;
        const list = pub.fishList ?? [];
        console.log(
          "[MarryClick][v2] 公開資料",
          fr.userId,
          "gameId=",
        ...
        // 把每條魚的關鍵欄位也印一下（看 stage / isMarried / isDead 有沒有帶到）
        for (const f of list) {
          console.log(
            " [MarryClick][v2] friend fish -> id=",
            f.id,
            "name=",
            f.name,
            "stage=",
          ...
          // <<偵錯期>> 先不過濾，全部放進 raw，稍後交給對話框去做 isCandidate 篩選
          friendCandidatesRaw.push({ ownerGameId, fish: f });
        }
      }

      console.log("[MarryClick][v2] 彙整完成（未過濾），朋友魚總數 =", friendCandidatesRaw.length);
    } catch (e) {
      console.error("[MarryClick][v2] 聚合朋友候選時發生錯誤", e);
    }

    // ===== 3) 注入 provider（用「快照」即可，因為此時我們已經抓齊資料）=====
    this.chooseDialog.setFriendCandidatesProvider(() => {
      console.log("[ChooseDialog][v2] 取用 friendCandidatesRaw，長度 =", friendCandidatesRaw.length);
      return friendCandidatesRaw;
    });
